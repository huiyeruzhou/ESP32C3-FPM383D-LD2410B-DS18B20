 ...
 *
 * File: main.cpp
 * Description: the main entry of LD2410B
 * Version: V1.0.0
 * Date: 2023/08/23
 * Author: YuHongli
 * Revision History:
...
__attribute__((unused)) static const char *TAG = "LD2410B Server";
...
void stub(void *ctx)
{
    // get ip and port of Anroid Device, create RPC Client
    AbilityContext *speakerContext = reinterpret_cast<AbilityContext *>(ctx);
    auto updateClient = new sensor_UpdateService_Client(speakerContext->getConnectIP(), speakerContext->getConnectPort());
    updateClient->open();
    TickType_t xLastWakeTime;
    const TickType_t xDelay80ms = pdMS_TO_TICKS(80);
    getDistance(NULL);
    int fail_cnt = 0;
    // run is the flag of the task
    while (run) {
...
int GPIO_Init(void)
{
    ...
    gpio_isr_handler_add(GPIO_NUM_0, gpio_isr_handler, reinterpret_cast<void *>(GPIO_NUM_0));
...
extern "C" void app_main(void)
{
    ...
    serial_init(256000);
    int max_len = 127;
    char *broadcast = reinterpret_cast<char *>(malloc((max_len + 1) * sizeof(char)));
