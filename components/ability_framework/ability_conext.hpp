 ...
 *
 * File: ability_conext.hpp
 * Description: the ability context class
 * Version: V1.0.0
 * Date: 2023/08/23
 * Author: YuHongli
 * Revision History:
...
class AbilityContext {
   private:
    ...
    // every row is coresponding to a status, every column is coresponding to a cmd
   ...
   public:
    ...
    const char *getConnectIP() { return connectIp; }
    void setConnectPort(uint16_t portArg) { connectPort = portArg; }
    uint16_t getConnectPort() { return connectPort; }
    ...
    const char *getAbilityRunningState()
    {
        ...
        snprintf(buf, max_len, abilityRunningStateFormat, abilityName, abilityPort, last_update.tv_sec, lifecyclePort, getStatusString());
        return buf;
    }
