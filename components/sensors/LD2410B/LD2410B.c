...
// F4 F3 F2 F1 0D 00 02 AA /*8头部*/02 /*9类型*/ 51  00 /*11运动物体距离*/
// 00/*12活力*/ 00 00 /*14静止物体距离*/3B/*15静止活力*/ 00 00/*17探测距离*/  55
// 00 F8 F7 F6 F5/*23尾部*/
/**
 * @brief 获取运动物体的距离
...
uint8_t getDistance(uint16_t *distance) {
  ...
  if (distance != NULL) {
    *distance =
        (PS_ReceiveBuffer[head + 16] << 8) | PS_ReceiveBuffer[head + 15];
  } else {
...
uint8_t startConfigure() {
  uint8_t data[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x04, 0x00, 0xFF,
  ...
  uint8_t ack[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x08, 0x00, 0xFF, 0x01, 0x00,
...
uint8_t stopConfigure() {
  uint8_t data[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x02, 0x00,
  ...
  uint8_t ack[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x04, 0x00, 0xFE,
...
uint8_t configSensity(uint32_t sensity) {
  ...
  // 判断是否成功
  int len = serial_receive(14, PS_ReceiveBuffer, 5000);
  uint8_t ack[] = {0xFD, 0xFC, 0xFB, 0xFA, 0x04, 0x00, 0x64,
                   0x01, 0x00, 0x00, 0x04, 0x03, 0x02, 0x01};
  for (int i = 0; i < len; i++) {
    // 找到帧头
    if (PS_ReceiveBuffer[i] == ack[0]) {
