...
5. 通信协议及进一步开发
  ...
  "FPM383C用户手册V1.2"(我提供的链接)和"FPM383C模组通信协议_V1.2"是两套东西, 经过串口通讯检验第一套绝大部分指令完全可用.至于通信协议里的另一套协议,也可以使用,真的不知道是什么关系,我也大受震撼,建议还是用第一套协议.我这里也没有提供"通信协议"的任何链接和文件,怕混淆,可以去[官网](https://r0.hlktech.com/Mobile/Download)查看FPM383C的相关资料
  [串口工具](https://r0.hlktech.com/download/HLK-FPM383C/3/%E4%B8%B2%E5%8F%A3%E6%B5%8B%E8%AF%95%E5%B7%A5%E5%85%B7.zip)里的默认指令是正确的,可以用来控制LED灯.

  通信协议的帧头是:
  `F1 1F E2 2E B6 6B A8 8A`用户手册中的通信协议,我调试过的通信协议,比较简洁易用,也是本代码中的指令
  `EF 01`通信协议中的通信协议, 我只尝试过一两个指令,感觉讲的不是很清楚.
